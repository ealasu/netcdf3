...
fn test_read_file_nc3_classic() {
    ...
    assert_eq!(Some(String::from("Example of NETCDF3_CLASSIC file")),               data_set.get_global_attr_as_string("title"));
    assert_eq!(Some(String::from("CF-1.8")),                                        data_set.get_global_attr_as_string("Conventions"));
...
fn test_read_file_nc3_classic_open_with_truncated_file() {
...

    {
        ...
        // Open the NetCDF-3 file
        let reading_res = FileReader::open(input_data_file_path);
...
fn test_read_file_nc3_64bit_offset() {
    ...
    let data_set: &DataSet = file_reader.data_set();
    assert_eq!(2,                                                                   data_set.num_global_attrs());
    assert_eq!(Some(String::from("Example of NETCDF3_64BIT_OFFSET file")),          data_set.get_global_attr_as_string("title"));
    assert_eq!(Some(String::from("CF-1.8")),                                        data_set.get_global_attr_as_string("Conventions"));
    check_temperatures_definition(file_reader.data_set());
...
fn check_temperatures_definition(data_set: &DataSet)
{
    ...
    // `longitude`
    {
        ...
        assert_eq!(Some(LONGITUDE_VAR_LEN),                         data_set.dim_size(LONGITUDE_VAR_NAME));
        assert_eq!(Some(DimensionType::FixedSize),                  data_set.dim_type(LONGITUDE_VAR_NAME));
    }
    // `time`
    {
        assert_eq!(true,                                            data_set.has_dim(TIME_VAR_NAME));
        assert_eq!(Some(TIME_VAR_LEN),                              data_set.dim_size(TIME_VAR_NAME));
        assert_eq!(Some(DimensionType::UnlimitedSize),              data_set.dim_type(TIME_VAR_NAME));
    }
...
fn check_temperatures_data(file_reader: &mut FileReader<std::fs::File>)
{
...
fn test_read_file_nc_fill_values() {
    ...
    // Copy bytes to a temporary file
    // ------------------------------
    let (tmp_dir, input_data_file_path) = copy_bytes_to_tmp_file(NC3_FILL_VALUES_FILE_BYTES, NC3_FILL_VALUES_FILE_NAME);
