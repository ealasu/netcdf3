...
fn test_file_writer_write_record_i8_errors() {
    ...
    // First create a new NetCDF-3 file and try to write wrong invalid records
    {
        ...
        assert_eq!(
            WriteError::RecordIndexExceeded{index: UNLIM_DIM_SIZE, num_records: UNLIM_DIM_SIZE},
            file_writer.write_record_i8(VAR_I8_NAME, UNLIM_DIM_SIZE, &RECORD_I8_DATA[..]).unwrap_err()
        );

        assert_eq!(
            WriteError::RecordMismatchDataLength{var_name: VAR_I8_NAME.to_string(), req: FIXED_DIM_SIZE, get: FIXED_DIM_SIZE-1},
            file_writer.write_record_i8(VAR_I8_NAME, 1, &RECORD_I8_DATA[..FIXED_DIM_SIZE-1]).unwrap_err()
        );
...
fn test_file_writer_write_record_u8_errors() {
    ...
    // First create a new NetCDF-3 file and try to write wrong invalid records
    {
        ...
        assert_eq!(
            WriteError::RecordIndexExceeded{index: UNLIM_DIM_SIZE, num_records: UNLIM_DIM_SIZE},
            file_writer.write_record_u8(VAR_U8_NAME, UNLIM_DIM_SIZE, &RECORD_U8_DATA[..]).unwrap_err()
        );

        assert_eq!(
            WriteError::RecordMismatchDataLength{var_name: VAR_U8_NAME.to_string(), req: FIXED_DIM_SIZE, get: FIXED_DIM_SIZE-1},
            file_writer.write_record_u8(VAR_U8_NAME, 1, &RECORD_U8_DATA[..FIXED_DIM_SIZE-1]).unwrap_err()
        );
...
fn test_file_writer_write_record_i16_errors() {
    ...
    // First create a new NetCDF-3 file and try to write wrong invalid records
    {
        ...
        assert_eq!(
            WriteError::RecordIndexExceeded{index: UNLIM_DIM_SIZE, num_records: UNLIM_DIM_SIZE},
            file_writer.write_record_i16(VAR_I16_NAME, UNLIM_DIM_SIZE, &RECORD_I16_DATA[..]).unwrap_err()
        );

        assert_eq!(
            WriteError::RecordMismatchDataLength{var_name: VAR_I16_NAME.to_string(), req: FIXED_DIM_SIZE, get: FIXED_DIM_SIZE-1},
            file_writer.write_record_i16(VAR_I16_NAME, 1, &RECORD_I16_DATA[..FIXED_DIM_SIZE-1]).unwrap_err()
        );
...
fn test_file_writer_write_record_i32_errors() {
    ...
    // First create a new NetCDF-3 file and try to write wrong invalid records
    {
        ...
        assert_eq!(
            WriteError::RecordIndexExceeded{index: UNLIM_DIM_SIZE, num_records: UNLIM_DIM_SIZE},
            file_writer.write_record_i32(VAR_I32_NAME, UNLIM_DIM_SIZE, &RECORD_I32_DATA[..]).unwrap_err()
        );

        assert_eq!(
            WriteError::RecordMismatchDataLength{var_name: VAR_I32_NAME.to_string(), req: FIXED_DIM_SIZE, get: FIXED_DIM_SIZE-1},
            file_writer.write_record_i32(VAR_I32_NAME, 1, &RECORD_I32_DATA[..FIXED_DIM_SIZE-1]).unwrap_err()
        );
...
fn test_file_writer_write_record_f32_errors() {
    ...
    // First create a new NetCDF-3 file and try to write wrong invalid records
    {
        ...
        assert_eq!(
            WriteError::RecordIndexExceeded{index: UNLIM_DIM_SIZE, num_records: UNLIM_DIM_SIZE},
            file_writer.write_record_f32(VAR_F32_NAME, UNLIM_DIM_SIZE, &RECORD_F32_DATA[..]).unwrap_err()
        );

        assert_eq!(
            WriteError::RecordMismatchDataLength{var_name: VAR_F32_NAME.to_string(), req: FIXED_DIM_SIZE, get: FIXED_DIM_SIZE-1},
            file_writer.write_record_f32(VAR_F32_NAME, 1, &RECORD_F32_DATA[..FIXED_DIM_SIZE-1]).unwrap_err()
        );
...
fn test_file_writer_write_record_f64_errors() {
    ...
    // First create a new NetCDF-3 file and try to write wrong invalid records
    {
        ...
        assert_eq!(
            WriteError::RecordIndexExceeded{index: UNLIM_DIM_SIZE, num_records: UNLIM_DIM_SIZE},
            file_writer.write_record_f64(VAR_F64_NAME, UNLIM_DIM_SIZE, &RECORD_F64_DATA[..]).unwrap_err()
        );

        assert_eq!(
            WriteError::RecordMismatchDataLength{var_name: VAR_F64_NAME.to_string(), req: FIXED_DIM_SIZE, get: FIXED_DIM_SIZE-1},
            file_writer.write_record_f64(VAR_F64_NAME, 1, &RECORD_F64_DATA[..FIXED_DIM_SIZE-1]).unwrap_err()
        );
